...
class VideoRecurrentModelPruneL1(VideoBaseModel):
    ...
    def __init__(self, opt):
        super(VideoRecurrentModelPruneL1, self).__init__(opt)
        self.opt = opt
        if self.is_train:
    ...
    def Prune(self,args,opt,train_sampler,prefetcher,val_loaders,resume_state):
        ...
        if resume_state:
            ...
            self.pruner.pruned_wg_pre = tp["pruned_wg_pre"]
            self.pruner.kept_wg_pre = tp["kept_wg_pre"]
            self.pruner.kept_wg_pre = tp["kept_wg_pre"]
            self.pruner.cal_pr()
            self.net_g = self.pruner.prune(resume_state)
    ...
    def Prune_final(self):
        self.pruner.model = copy.deepcopy(self.net_g)
        self.net_g = self.pruner.prune_final()
    ...
    def optimize_parameters(self, current_iter):
        ...
        super(VideoRecurrentModelPruneL1, self).optimize_parameters(current_iter)
